...
import (
	...
	internalErrs "gitlab.com/distributed_lab/json-api-connector/horizon/errors"
...
func (c *Client) Get(endpoint *url.URL) ([]byte, error) {
	fullEndpoint, err := c.resolveURL(endpoint)
	if err != nil {
		return nil, err
	}
...
func (c *Client) PostJSON(endpoint *url.URL, req interface{}) (statusCode int, response []byte, err error) {
	return c.PostJSONWithContext(endpoint, req, context.TODO())
}

func (c *Client) PostJSONWithContext(endpoint *url.URL, req interface{}, ctx context.Context) (statusCode int, response []byte, err error) {
	...
	resolvedEndpoint, err := c.resolveURL(endpoint)
	if err != nil {
		return 0, nil, errors.Wrap(err, "Failed to resolve url")
	}
	fields["resolved_url"] = resolvedEndpoint
...
func (c *Client) resolveURL(endpoint *url.URL) (string, error) {
	u := *c.base
	basePath := u.Path
...
func (c *Client) Do(request *http.Request) ([]byte, error) {
	...
	if c.signer != nil {
		// TODO move to proper keypair
		seed := depkeypair.MustParse(c.signer.Seed())
		signcontrol.SignRequest(request, seed, seed)
	}
