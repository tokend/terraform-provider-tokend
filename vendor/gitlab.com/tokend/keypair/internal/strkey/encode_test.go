...
func TestEncode(t *testing.T) {
	t.Run("valid", func(t *testing.T) {
		t.Run("address", func(t *testing.T) {
			...
			got, err := Encode(VersionByteAccountID, raw)
			assert.NoError(t, err)
			assert.Equal(t, expected, got)
		})

		t.Run("balance", func(t *testing.T) {
			expected := "BAQT7PE345GEJVOOWPEWKSRZRETVDIHUNBHWDU3FQYWEYLTOL64RH75C"
			raw := []byte{
			...
			got, err := Encode(VersionByteBalanceID, raw)
			assert.NoError(t, err)
			assert.Equal(t, expected, got)
		})
		...
		t.Run("seed", func(t *testing.T) {
			...
			got, err := Encode(VersionByteSeed, raw)
			assert.NoError(t, err)
			assert.Equal(t, expected, got)
		})
	})
	...
	t.Run("fails", func(t *testing.T) {
		t.Run("invalid version byte", func(t *testing.T) {
			...
			_, err := Encode(VersionByte(2), raw)
			assert.Error(t, err)
		})
	})
}
